...
1. Sensores que en el último mes no han transmitido ningún dato.
2. Sensores que tengan muy baja transmisión (< 10 transmisiones/mes)
3. Sensores con < 10 % batería
4. Sensores que, es su espectro, muestren un comportamiento defectuoso
...
# Cantidad de sensores sin data en un mes
length(setdiff(substr(MAC$numDMI,3,6), str_pad(names(CNas), 4, pad = "0")))
